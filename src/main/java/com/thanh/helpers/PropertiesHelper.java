...
public class PropertiesHelper {
    ...
    public static Properties loadAllFiles() {
        LinkedList<String> files = new LinkedList<>(); //Link ở trong list
    ...
    public static void setDefaultFile() {
        properties = new Properties(); //Khai báo
        try {
            linkFile = Helpers.getCurrentDir() + relPropertiesFilePathDefault;
            file = new FileInputStream(linkFile); //đọc file properties
            properties.load(file);// load properties đó lên
            file.close();
        } catch (Exception e) {
            e.printStackTrace();
        }
