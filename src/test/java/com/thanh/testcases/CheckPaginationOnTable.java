...
public class CheckPaginationOnTable extends BaseTest {
    ...
    public void testCheckPaginationOnTable() {
        ...
        for (String strItem : strTotal.split("\\s")) { //căt chuỗi theo 1 -1 kí tự
            list.add(strItem);
        }

        System.out.println(list);

        int itemTotal = Integer.parseInt(list.get(5)); //Tổng số lượng phần tử
        System.out.println("Tổng số item: " + itemTotal);
        int pageTotal = itemTotal / itemTotalOnePage; //Tổng số lượng phần tử/ số phần tử trong 1 trang
